...
export default function Colors () {
  ...
  useEffect(() => {
    ...
    ff.sort((a,b) => {
      ...
      case (sort === "red value"):
        switch (true){
        case (ared - (agreen + ablue)/2 > bred - (bgreen + bblue)/2):
            return -1;
        case (ared - (agreen + ablue)/2 < bred - (bgreen + bblue)/2):
            return 1;
          default:
      ...
      case (sort === "green value"):
        switch (true){
        case (agreen - (ared + ablue)/2 > bgreen - (bred + bblue)/2):
            return -1;
        case (agreen - (ared + ablue)/2 < bgreen - (bred + bblue)/2):
            return 1;
          default:
      ...
      default:
        switch (true){
        case (ablue - (ared + agreen)/2 > bblue - (bred + bgreen)/2):
            return -1;
        case (ablue - (ared + agreen)/2 < bblue - (bred + bgreen)/2):
            return 1;
          default:
  ...
  return (
    ...
    >
      ...
      <SortSearch sortOptions={sortOptions} search={search} setSearch={setSearch} sort={sort} setSort={setSort}/>

      <motion.div className={styles.table} layout transition={{ duration: 0.3 }}>
        {
          colors.map(color => {
            return (
            <div key = {color.name} className={styles.row}
                style={{
                  color: dark ? "#fff" : "#000",
            ...
            >
              <div>
                <div className={styles.name}>
                  {color.name}
                  <div onClick={() => handleClick(color.name)}>
                      {copyicon}
                  </div>
                </div>
                ...
                <div className={styles.hsl}>
                  hsl({color.hsl})
                  <div onClick={() => handleClick(`hsl(${color.hsl})`)}>
                      {copyicon}
                  </div>
                </div>
              </div>
